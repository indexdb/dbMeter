...
#define VERSION "v1.0"
#define APP "dBmeter"
const char* urlBase = "http://updateserver:5002/update";
...
SOS_IIR_Filter DC_BLOCKER(1.0,{{-1.0, 0.0, +0.9992, 0}});
// 
...
SOS_IIR_Filter ICS43434(0.477326418836803,{{+0.96986791463971267, 0.23515976355743193, -0.06681948004769928, -0.00111521990688128},
        { -1.98905931743624453, 0.98908924206960169, +1.99755331853906037, -0.99755481510122113 }});
...
SOS_IIR_Filter ICS43432(0.457337023383413,{ // Second-Order Sections {b1, b2, -a1, -a2}
        {-0.544047931916859, -0.248361759321800, +0.403298891662298, -0.207346186351843},
        { -1.909911869441421, +0.910830292683527, +1.790285722826743, -0.804085812369134 },
        { +0.000000000000000, +0.000000000000000, +1.148493493802252, -0.150599527756651 }});
...
*/
SOS_IIR_Filter INMP441(1.00197834654696,{{-1.986920458344451, +0.986963226946616, +1.995178510504166, -0.995184322194091}});
...
*/
SOS_IIR_Filter IM69D130(1.00124068496753,{{-1.0, 0.0, +0.9992, 0}, // DC blocker, a1 = -0.9992
        { -1.994461610298131, 0.994469278738208, +1.997675693595542, -0.997677044195563 }});
...
SOS_IIR_Filter SPH0645LM4H_B_RB(1.00123377961525,{{-1.0, 0.0, +0.9992, 0}, // DC blocker, a1 = -0.9992
        { -1.988897663539382, +0.988928479008099, +1.993853376183491, -0.993862821429572 }});
//
// Weighting filters
//
...
SOS_IIR_Filter A_weighting(0.169994948147430,{{-2.00026996133106, +1.00027056142719, -1.060868438509278, -0.163987445885926},
        { +4.35912384203144, +3.09120265783884, +1.208419926363593, -0.273166998428332 },
        { -0.70930303489759, -0.29071868393580, +1.982242159753048, -0.982298594928989 }});
//
...
SOS_IIR_Filter C_weighting(-0.491647169337140,{{+1.4604385758204708, +0.5275070373815286, +1.9946144559930252, -0.9946217070140883},
        { +0.2376222404939509, +0.0140411206016894, -1.3396585608422749, -0.4421457807694559 },
        { -2.0000000000000000, +1.0000000000000000, +0.3775800047420818, -0.0356365756680430 }});
...
void connect_mqtt()
{
    ...
    while (!mqttClient.connected())
    {
        ...
        if (mqttClient.connect(clientId.c_str(), mqtt_user, mqtt_password))
        {
            ...
            // mqttClient.publish("homeassistant/sensor/dBmeter/info", clientId.c_str()); // publish first message
            Serial.print("MQTT .. ");
        }
        else
        {
...
void setupWifi() {
    ...
    baseUniqueId =  String(mac[0], HEX) + String(mac[1], HEX) + String(mac[2], HEX) + String(mac[3], HEX) + String(mac[4], HEX) + String(mac[5], HEX);
    Serial.println("");
...
void ShowLogo ()
{
  ...
  display.print("dBmeter");
  display.setTextSize(1);
  display.setCursor(100,43);
  display.print(VERSION);
  display.display();
}
void ShowdB (const char * msg)
{
  display.clearDisplay();
  display.setTextColor(WHITE);
  display.setTextSize(1);
  display.setFont(&FreeSerifBold24pt7b);  // 设置大字体
  display.setCursor(15+5,18+20+7);
  display.print(msg);
...
void setup() {
    ...
    String jsonPayload;
    String configTopic = "homeassistant/sensor/dBmeter_"+baseUniqueId+"/config";
    
    doc["device_class"] = "sound_pressure";
    doc["name"] = "dBmeter";
    doc["state_topic"] = String("homeassistant/sensor/dBmeter_"+baseUniqueId+"/soundlevel");
    doc["unique_id"] = baseUniqueId;
    doc["unit_of_measurement"] = "dB";
    //doc["value_template"] = "{{ value_json.soundlevel | round(2) }}";
  
    serializeJson(doc, jsonPayload);
    
    mqttClient.publish(configTopic.c_str(), jsonPayload.c_str(), true);
    ...
    while (xQueueReceive(samples_queue, &q, portMAX_DELAY)) {
        ...
        if (Leq_samples >= SAMPLE_RATE * LEQ_PERIOD) {
            ...
            if (abs(Leq_dB-baseDB) > dBdifference) {
                ...
                String msgOne = (String)Leq_dB; //"{\"soundlevel\":"+(String)Leq_dB+"}";                
                int msgLen = msgOne.length();
                mqttClient.beginPublish(("homeassistant/sensor/dBmeter_"+baseUniqueId+"/soundlevel").c_str(), msgLen, true);
                ...
                mqttClient.beginPublish(("homeassistant/sensor/dBmeter_"+baseUniqueId+"/rssi").c_str(), msgLen, true);
                mqttClient.print(strValue);
                mqttClient.endPublish();
                char result[20];
                sprintf(result, "%.1f", round(Leq_dB*10)/10);
                ShowdB(result);
                baseDB=Leq_dB;
            }
            // Debug only
        ...
        // Serial.print(digitalRead(ONOFF_KEY_PIN));
        
        if (digitalRead(ONOFF_KEY_PIN)==HIGH){        
          delay(20);
          if (digitalRead(ONOFF_KEY_PIN)==HIGH){
            digitalWrite(ONOFF_PIN,HIGH);
            display.clearDisplay();
            display.display();
            while(digitalRead(ONOFF_KEY_PIN)==HIGH);
          }
        }
